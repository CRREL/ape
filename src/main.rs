...
fn main() {
    ...
    if let Some(matches) = matches.subcommand_matches("sop") {
        ...
        if let Some(matches) = matches.subcommand_matches("create") {
            ...
            let velocities = grid.calculate_velocities(
                value_t!(matches, "threads", usize).ok(),
                value_t!(matches, "sigma2", f64).ok(),
            ).unwrap();
            ...
            let velocities: Vec<velocities::Velocity> = serde_json::from_reader(infile).unwrap();
